...
var ConfigurationWhere = struct {
	...
	ID:              whereHelperint64{field: "\"ms_graph\".\"configuration\".\"id\""},
	...
	Active:          whereHelpernull_Bool{field: "\"ms_graph\".\"configuration\".\"active\""},
	Enable:          whereHelpernull_Bool{field: "\"ms_graph\".\"configuration\".\"enable\""},
	ProjectIds:      whereHelpertypes_StringArray{field: "\"ms_graph\".\"configuration\".\"project_ids\""},
}
...
func Configurations(mods ...qm.QueryMod) configurationQuery {
	mods = append(mods, qm.From("\"ms_graph\".\"configuration\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"ms_graph\".\"configuration\".*"})
	}
...
func FindConfiguration(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Configuration, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"ms_graph\".\"configuration\" where \"id\"=$1", sel,
	)
...
func (o *Configuration) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ms_graph\".\"configuration\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ms_graph\".\"configuration\" %sDEFAULT VALUES%s"
		}
...
func (o *Configuration) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"ms_graph\".\"configuration\" SET %s WHERE %s",
...
func (o ConfigurationSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"ms_graph\".\"configuration\" SET %s WHERE %s",
...
func (o *Configuration) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"ms_graph\".\"configuration\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Configuration) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ms_graph\".\"configuration\" WHERE \"id\"=$1"
...
func (o ConfigurationSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ms_graph\".\"configuration\" WHERE " +
...
func (o *ConfigurationSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"ms_graph\".\"configuration\".* FROM \"ms_graph\".\"configuration\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, configurationPrimaryKeyColumns, len(*o))
...
func ConfigurationExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"ms_graph\".\"configuration\" where \"id\"=$1 limit 1)"
